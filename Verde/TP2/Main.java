...
class Main {
    public static void main(String[] args) {
        ...
        // Second part: search titles until "FIM"
        long startTime = System.currentTimeMillis();
        int comparacoes = 0;

        String titlePesquisa = sc.nextLine();
        while (!titlePesquisa.equals("FIM")) {
            boolean encontrado = false;

            for (int j = 0; j < i; j++) {
                comparacoes++;
                if (lista[j] != null && lista[j].getTitle().equals(titlePesquisa)) {
                    encontrado = true;
                    break;
                }
            }
        ...
        try {
            BufferedWriter writer = new BufferedWriter(new FileWriter("858399_sequencial.txt"));
            writer.write("858399\t" + tempoExecucao + "\t" + comparacoes);
            writer.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
