...
class Main {
    public static void sort(Show[] lista, int n) {
        ...
        for (int i = 1; i < n; i++) {
            ...
            while (j >= 0) {
                comparacoes++;
                boolean precisaTrocar = lista[j].getType().compareTo(tmp.getType()) > 0 ||
                    (lista[j].getType().compareTo(tmp.getType()) == 0 &&
                     lista[j].getTitle().toLowerCase().compareTo(tmp.getTitle().toLowerCase()) > 0);
        ...
        try (FileWriter fw = new FileWriter("858399")) {
            fw.write("matricula\t" + tempoMs + "\t" + comparacoes + "\t" + movimentacoes);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
