    ...
    public static void main(String[] args) {
        ...
        while (!lerId.equals("FIM"))  {
            try (BufferedReader br = new BufferedReader(new FileReader(path))) {
                String line = br.readLine(); // ignora primeira linha
        ...
        double tempo = (end - start) / 1_000_000.0; // tempo em ms
        ...
        try {
            FileWriter fw = new FileWriter("858399");
            fw.write("123456\t" + comparacoes + "\t" + movimentacoes + "\t" + String.format(Locale.US, "%.3f", tempo) + "\n");
            fw.close();
        } catch (IOException e) {
            e.printStackTrace();
        }
...
class Show {
    ...
    public void imprimir() {
        System.out.printf("=> %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s ##\n",
            ...
            date, (year == 0 ? "NaN" : year), rating, duration, Arrays.toString(listed));
    }
