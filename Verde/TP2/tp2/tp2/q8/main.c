...
void insercao(Show lista[], int cor, int h, int n, int* comparacoes) {
    for (int i = (h + cor); i < n; i += h) {
        Show tmp = lista[i];
        int j = i - h;
        while ((j >= 0) && (
            (*comparacoes)++ >= 0 && strcmp(lista[j].type, tmp.type) > 0 ||
            ((*comparacoes)++ >= 0 && strcmp(lista[j].type, tmp.type) == 0 &&
             strcasecmp(lista[j].title, tmp.title) > 0))) {
            lista[j + h] = lista[j];
            j -= h;
        }
        lista[j + h] = tmp;
...
void sort(Show lista[], int n, int* comparacoes) {
    int h = 1;
    do {
        h = (h * 3) + 1;
    } while (h < n);
    do {
        ...
        for (int cor = 0; cor < h; cor++) {
            insercao(lista, cor, h, n, comparacoes);
        }
    } while (h != 1);
}

int main() {
    char path[] = "/tmp/disneyplus.csv";  // ou "../disneyplus.csv" dependendo do ambiente
    Show lista[1369];
    int i = 0;
    ...
    while (strcmp(lerId, "FIM") != 0) {
        ...
        fgets(line, MAX_LINE_LENGTH, file); // pula cabeçalho
    ...
    FILE* log = fopen("858399_shellsort.txt", "w");
    if (log != NULL) {
        fprintf(log, "13649365\t%lf\t%d\n", tempo, comparacoes);
        fclose(log);
    }
